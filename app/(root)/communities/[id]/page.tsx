...
const Page: FC<CommunitiesProps> = async ({ params: { id } }) => {
          ...
          <TabsList className="tab">
            {communityTabs.map((tab) => (
              <TabsTrigger key={tab.label} value={tab.value} className="tab">
                ...
                {tab.label === "Threads" && <p className="ml-1 rounded-sm bg-light-4 px-2 py-1 !tiny-text-medium text-light-2">{communityDetails?.threads?.length}</p>}
              </TabsTrigger>
            ))}
          </TabsList>
