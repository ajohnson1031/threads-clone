...
const fetchPosts = async ({ pageNumber = 1, pageSize = 20 }: { pageNumber: number; pageSize: number }) => {
  connectToDB();
...
const deleteThread = async ({ id, path }: { id: string; path: string }): Promise<void> => {
  try {
    connectToDB();
...
const addCommentToThread = async ({ threadId, commentText, userId, path }: { threadId: string; commentText: string; userId: string; path: string }) => {
  connectToDB();
...
export { addCommentToThread, createThread, deleteThread, fetchPosts, fetchThreadById };
